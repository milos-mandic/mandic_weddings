...
export default function Home() {
  ...
  return (
    <main>
      {/* Hero Section */}
      <section id="hero" style={{ minHeight: '90vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
      ...
      {/* Wedding Timeline */}
      <section id="timeline" style={{ background: '#e7e4dd', padding: '4rem 0', minHeight: '120vh', position: 'relative' }}>
        <h3 style={{ textAlign: 'center', fontFamily: 'Arial, sans-serif', fontWeight: 700, fontSize: '2rem', marginBottom: '2rem' }}>Wedding Stories</h3>
        <div style={{ position: 'relative', maxWidth: 1100, margin: '4rem auto 0 auto', minHeight: 700 }}>
          {/* Vertical timeline line (now 2px) */}
          <div style={{ position: 'absolute', left: '50%', top: 0, bottom: 0, width: 2, background: '#222', transform: 'translateX(-50%)', zIndex: 0, borderRadius: 1 }}></div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '6rem', position: 'relative', zIndex: 1 }}>
            {weddings.map((w, idx) => {
              const isLeft = idx % 2 === 0;
              return (
                <div key={w.title} style={{ display: 'flex', flexDirection: 'row', alignItems: 'center', justifyContent: 'center', minHeight: 220, position: 'relative' }}>
                  {/* Left side: Name or Video */}
                  {isLeft ? (
                    <>
                      {/* Name + horizontal line (now 2px) */}
                      <div style={{ flex: 1, display: 'flex', justifyContent: 'flex-end', alignItems: 'center', paddingRight: 32 }}>
                        <span style={{ fontFamily: 'Enriqueta, serif', fontSize: '1.3rem', fontWeight: 500, marginRight: 16 }}>{w.title}</span>
                        <div style={{ width: 'calc(50% - 110px)', height: 2, background: '#222', borderRadius: 1 }}></div>
                      </div>
                      {/* Timeline dot (center) */}
                      {/* Removed the circle */}
                      {/* Video */}
                      <div style={{ flex: 1, display: 'flex', justifyContent: 'flex-start', position: 'relative', paddingLeft: 64 }}>
                        <div style={{ width: 604, height: 340, background: '#fff', boxShadow: '0 2px 12px rgba(0,0,0,0.07)', overflow: 'hidden', position: 'relative' }}>
                          <iframe width="604" height="340" src={w.video} title={w.title} frameBorder="0" allowFullScreen style={{ display: 'block' }}></iframe>
                        </div>
                      </div>
                    </>
                  ) : (
                    <>
                      {/* Video */}
                      <div style={{ flex: 1, display: 'flex', justifyContent: 'flex-end', position: 'relative', paddingRight: 64 }}>
                        <div style={{ width: 604, height: 340, background: '#fff', boxShadow: '0 2px 12px rgba(0,0,0,0.07)', overflow: 'hidden', position: 'relative' }}>
                          <iframe width="604" height="340" src={w.video} title={w.title} frameBorder="0" allowFullScreen style={{ display: 'block' }}></iframe>
                        </div>
                      </div>
                      ...
                      <div style={{ flex: 1, display: 'flex', justifyContent: 'flex-start', alignItems: 'center', paddingLeft: 32 }}>
                        <div style={{ width: 'calc(50% - 110px)', height: 2, background: '#222', borderRadius: 1 }}></div>
                        <span style={{ fontFamily: 'Enriqueta, serif', fontSize: '1.3rem', fontWeight: 500, marginLeft: 16 }}>{w.title}</span>
                      </div>
                    </>
                  )}
                </div>
      ...
      <section id="about" style={{ background: '#fff', padding: '4rem 0', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', justifyContent: 'center' }}>
          <img src="/bio-image.jpg" alt="About" style={{ margin: '1rem', width: '300px', height: '450px', objectFit: 'cover' }} />
